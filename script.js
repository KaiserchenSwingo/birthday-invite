...
// === PURE DDD:HH:MM:SS COUNTDOWN (22.12.2025 00:00) ===
(function(){
  const el=document.getElementById('countdown'); if(!el) return;
  el.innerHTML=''; const cd=document.createElement('span'); cd.className='cd'; el.appendChild(cd);

  let target=new Date('2025-12-22T00:00:00+01:00');
  if (el.dataset?.target){ const t=new Date(el.dataset.target); if(!isNaN(+t)) target=t; }
