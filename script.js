...
(function(){
  ...
  function showApp(){ if(gate){gate.classList.add('hidden');gate.style.display='none';} if(app){app.classList.remove('hidden');app.style.removeProperty('display');} try{document.body.classList.remove('pin-locked');}catch(e){} }
  function showGate(){ if(gate){gate.classList.remove('hidden');gate.style.removeProperty('display');} if(app){app.classList.add('hidden');app.style.display='none';} try{document.body.classList.add('pin-locked');}catch(e){} }
  try{ if(localStorage.getItem(KEY)==='1') showApp(); else showGate(); }catch(e){ showGate(); }
  if(form){
  ...
  // Countdown to 22.12.2025 00:00
  (function(){
    const el=document.getElementById('countdown'); if(!el) return;
    let target=new Date('2025-12-22T00:00:00+01:00');
    if(el.dataset && el.dataset.target){ const t=new Date(el.dataset.target); if(!isNaN(+t)) target=t; }
    el.textContent=''; const cd=document.createElement('span'); cd.className='cd'; el.appendChild(cd);
    const p2=n=>String(n).padStart(2,'0'), p3=n=>String(n).padStart(3,'0');
    function tick(){ const diff=target-new Date(); if(diff<=0){ cd.textContent='000:00:00:00'; return;}
